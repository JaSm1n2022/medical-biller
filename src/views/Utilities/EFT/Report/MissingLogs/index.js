...
let isClaimsDone = false;
let isEftsDone = false;
let claimList = [];
let eftList = [];
function MissingLogs(props) {
  ...
  if (props.efts?.status === ACTION_STATUSES.SUCCEED) {
    eftList = props.efts.data;
    isEftsDone = true;
    props.resetListEfts();
  }
  ...
  const dataSourceHandler = (data) => {
    grandTotal = 0.0;
    let source = data;
    if (source && source.length) {
      source = MedicaidHandler.mapData(source);
    }
          ...
          <Card>
            <CardHeader color="success">
              <Grid container justifyContent="space-between">
                <h4 className={classes.cardTitleWhite}>Missing Logs</h4>
              </Grid>
            </CardHeader>
            <CardBody>
              <Grid
...
export default connect(mapStateToProps, mapDispatchToProps)(MissingLogs);
